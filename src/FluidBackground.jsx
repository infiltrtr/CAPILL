...
export default function FluidBackground({ baseColor }) {
  ...
  useEffect(() => {
    ...
    // 2. MOTOR DE COLOR (Deriva sombras y luces del color de la bolita)
    const base = new THREE.Color(baseColor);
    // Valley (Muy oscuro), Mid-Dark, Mid-Light (Base), Peak (Casi blanco)
    const c1 = base.clone().multiplyScalar(0.15); 
    const c2 = base.clone().multiplyScalar(0.5);  
    const c3 = base.clone();                      
    const c4 = base.clone().lerp(new THREE.Color(0xffffff), 0.6); 

    // 3. SHADERS (Tu código matemático exacto)
    const vertexShader = `
      varying vec2 vUv;
      void main() {
          vUv = uv;
    ...
    const fragmentShader = `
      ...
      uniform vec3 uLightPos; uniform float uDepth; uniform float uSpeed;
      uniform float uNoiseScale; uniform float uWarpAmount; uniform float uFoldFrequency;
      uniform float uAngle; uniform float uConnections; uniform float uShadowWidth;
    ...
    // Parámetros físicos inyectados directo al material
    ...
    // Protocolo de limpieza estricto para React
    return () => {
  ...
  }, [baseColor]); // Re-calcula si el color base cambia

  // Usamos un div absoluto y sin eventos de puntero para no bloquear los clics
